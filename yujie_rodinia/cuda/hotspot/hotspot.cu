...
__device__ int processed_thread_cnt;
__global__ void calculate_temp(int iteration,  //number of iteration
	...
	__syncthreads();
      ...
      // small block perform the calculation and switch on ``computed''
       atomicAdd(&processed_thread_cnt, 1);
      if (computed){
               
          temp_dst[index]= temp_t[ty][tx];		
      }
      if(processed_thread_cnt>=473088){
        // if(processed_thread_cnt%2048==0){
        // printf("cound: %d\n",processed_thread_cnt);
      }
}
		...
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows) 
{
	...
	for (t = 0; t < total_iterations; t+=num_iterations) {
            ...
            src = dst;
            dst = temp;
         printf("%d Maximum grid size: (%d, %d, %d)\n",num_iterations, dimGrid.x, dimGrid.y, dimGrid.z);
  printf("Maximum block size: (%d, %d, %d)\n", dimBlock.x, dimBlock.y, dimBlock.z);
        ...
        printf("!!Elapsed time in nanoseconds:%d\n ns", chrono::duration_cast<chrono::nanoseconds>(end_time - start_time).count());
